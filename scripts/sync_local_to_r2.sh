...
# 同步本地数据到 CloudFlare R2 的脚本
# 使用方法: ./scripts/sync_local_to_r2.sh
...
DATABASE_URI=$(grep -A 5 '^\[database\]' "$CONFIG_FILE" | grep 's3://' | sed 's/.*=\s*"\(.*\)".*/\1/' | sed 's/#\s*//' | tr -d '\r\n' | head -n 1)
...
echo "开始上传数据..."
echo "从: $LOCAL_PATH"
echo "到: ${REMOTE_NAME}:${BUCKET_NAME}/${REMOTE_PATH}"
echo ""

# 使用 rclone sync 同步数据（本地 -> 远程）
# --progress: 显示进度
# --verbose: 显示详细信息
# --transfers: 并行传输数量
# --checkers: 并行检查数量
...
echo ""
echo "上传完成！"
echo "本地数据已同步到 R2: ${BUCKET_NAME}/${REMOTE_PATH}"
